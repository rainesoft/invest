...
15. COMMODITY GEOPOLITICAL SUPPLY SHOCK OVERRIDE: For commodities (UKOIL, XAUUSD, XAGUSD), technical EMAs are SECONDARY to active supply disruption events. 
    ...
    - In such scenarios, you MUST NOT default to C-Tier simply because the price sits between EMAs. Instead, evaluate the geopolitical catalyst as the PRIMARY driver, and if it supports a directional bias (e.g., supply disruption = bullish oil), you MUST originate a trade setup with S-Tier confidence (90+) provided the structural R:R is viable.
