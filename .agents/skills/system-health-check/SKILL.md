...
3. Verify the output to ensure:
   ...
   - Adaptive Limit Inversion Guard (Section 1R & Section 3S) in `agent-trade` and `vps-poll` automatically converts crossed pending limit orders (`BUY LIMIT` $\ge$ market ask or `SELL LIMIT` $\le$ market bid) to market orders, fetching live candle prices via `fetchRecentBars` without throwing `ReferenceError: currentPrice is not defined` or MT5 `Code:10016` / `Code:10044` rejections. All Edge Functions strictly pass Deno TypeScript type checking.
...
4. If anomalies or status desyncs are detected, apply the reconciliation SQL from Section 2N, Section 3G, Section 3H, Section 3I, Section 3P, Section 3Q, Section 3R, and Section 3S of `docs/System Health Checklist.md` and report findings to the user.
