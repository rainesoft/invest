...
3. Verify the output to ensure:
   ...
   - Active Position Exposure & Stacking Concurrency Guard (Section 2N) is enforced in `agent-swing` (Layer -0.5), `agent-day`, and `agent-trade`, blocking duplicate concurrent open opportunities on the same symbol to prevent free margin exhaustion (`Code:10019`).
   ...
   - Institutional algorithmic execution filters (Section 3U) are enforced: Auction Market Theory Value Area pre-filter discards mid-range chop (30%-70% VA) in `agent-day`, Late-Week Swing Entry Gate freezes non-crypto 1D origination on late Thursday (>=16:00 UTC) and Friday in `agent-swing`, Floor Trader Pivot Alignment Guard prevents blind sub-pivot long and super-pivot short traps in `agent-day`, Wilder's ADX momentum scaling expands TP1 targets when $ADX \ge 30$, Pre-AI Cross-Asset Correlation Lock prevents contradictory exposure across indices/metals/energy/FX, Dynamic Limit Fill Urgency factor ($0.04\times\text{ ATR}$) eliminates missed momentum fills, Rollover Spread Gate halts non-crypto origination during 21:00-22:15 UTC spread spikes, and Hive Mind 1D Macro Directives enforce top-down multi-timeframe directional hierarchy.
...
4. If anomalies or status desyncs are detected, apply the reconciliation SQL from Section 2N, Section 3G, Section 3H, Section 3I, Section 3P, Section 3Q, Section 3R, Section 3S, Section 3T, and Section 3U of `docs/System Health Checklist.md` and report findings to the user.
