...
3. Verify the output to ensure:
   ...
   - Adaptive Limit Inversion Guard (Section 3S) in `agent-trade` and `vps-poll` automatically converts crossed pending limit orders (`BUY LIMIT` $\ge$ market ask or `SELL LIMIT` $\le$ market bid) to market orders, preventing MT5 `Code:10016` and `Code:10044` rejections.
