...
### 2. `agent-swing` (Technical Confluence)
...
### 3. `agent-trade` (Execution Desk / PAMM Router)
*The Chief Risk Officer (CRO).*
- **Schedule (Cron):** Every 5 Minutes (`3-59/5 * * * *`)
- **Role:** It acts as the final gatekeeper. It sweeps the database for `APPROVED` setups generated by `agent-swing` and runs them through strict risk management filters.
- **Action:** 
...
### 4. `position-manager` (Risk & Trade Management)
*The Trade Guardian.*
...
graph TD
    ...
    G -->|Risk Checks & Sizing| H(MetaAPI Broker)
